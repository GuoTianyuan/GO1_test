...
#define JOY_CMD_VELX_MAX 0.8         // m/s
...
#define JOY_STATE_TOTAL_STATES 2 // 3
// state list
// follow gait.info
// 0 stance             (default)
// 1 trot
...
class ROSJoyProcessor {
 public:
  ROSJoyProcessor(ros::NodeHandle& _nh, const std::string& gaitFile, const std::string& robotName) {
    ...
    _nh.param("/joystick_perceptive_start_button", joystick_perceptive_start_button, 2);
    _nh.param("/joystick_standup_button", joystick_standup_button, 0);
  ...
  void processJoy(double dt) {
    if (joy_cmd_ctrl_state_change_request) {
      ...
      } else if (joy_cmd_ctrl_state == 3) {
        gaitCommand = "bound";
      } else if (joy_cmd_ctrl_state == 4) {
        gaitCommand = "pronk";
      } else{
        gaitCommand = "stance";
      }
      
      if(joy_cmd_ctrl_stance_request){
        std::cout << "control request stance !" << std::endl;
        gaitCommand = "stance";
      }
      joy_cmd_ctrl_stance_request = false;

      std::cout << "gait name:" << gaitCommand << std::endl;
  ...
  void joy_callback(const sensor_msgs::Joy::ConstPtr& joy_msg) {
    ...
    // stance request
    if (joy_msg->buttons[joystick_standup_button] == 1)
      joy_cmd_ctrl_stance_request = true;
    ...
    if (joy_msg->buttons[joystick_start_button] == 1) {
      std::cout << "You have pressed the controller start button!!!!" << std::endl;
    ...
    if (joy_msg->buttons[joystick_perceptive_start_button] == 1) {
      std::cout << "You have pressed the perceptive controller start button!!!!" << std::endl;
      // set the string start_controllers to controllers/legged_controller
      switch_ctrl_srv.request.start_controllers = {"controllers/perceptive_controller"};
      switch_ctrl_srv.request.stop_controllers = {""};
      switch_ctrl_srv.request.strictness = switch_ctrl_srv.request.BEST_EFFORT;
      switch_ctrl_srv.request.start_asap = true;
      switch_ctrl_srv.request.timeout = 0.0;
    ...
    if (joy_msg->buttons[joystick_stop_button] == 1) {
      ...
      switch_ctrl_srv.request.stop_controllers = {"controllers/legged_controller", "controllers/perceptive_controller"};
      switch_ctrl_srv.request.strictness = switch_ctrl_srv.request.BEST_EFFORT;
      switch_ctrl_srv.request.start_asap = true;
      switch_ctrl_srv.request.timeout = 0.0;
      switch_ctrl_client.call(switch_ctrl_srv);
 ...
 private:
  ...
  bool joy_cmd_ctrl_stance_request = false;
  ...
  int joystick_perceptive_start_button;
  int joystick_standup_button;
