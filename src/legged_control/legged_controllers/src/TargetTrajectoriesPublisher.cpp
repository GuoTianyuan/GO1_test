...
TargetTrajectories cmdVelToTargetTrajectoriesWithTerrain(const vector_t& cmdVel, const SystemObservation& observation, const scalar_t &terrain_pitch_angle) {
  const vector_t currentPose = observation.state.segment<6>(6);
  const Eigen::Matrix<scalar_t, 3, 1> zyx = currentPose.tail(3);
  vector_t cmdVelRot = getRotationMatrixFromZyxEulerAngles(zyx) * cmdVel.head(3);
...
int main(int argc, char** argv) {
  ...
  TargetTrajectoriesPublisher target_pose_command(nodeHandle, gaitCommandFile, robotName, &goalToTargetTrajectories,
                                                  &cmdVelToTargetTrajectories, &cmdVelToTargetTrajectoriesWithTerrain);
